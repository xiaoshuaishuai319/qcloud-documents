...
腾讯云对象存储 COS 目前提供了 [COS 签名工具](https://cos5.cloud.tencent.com/static/cos-sign/) 供用户生成签名，如有需要可前往使用。同时各语言 SDK 提供**预签名 URL**，方便用户获取携带签名的 URL，并自行处理相关的请求，详情请参见各语言 SDK 的**预签名 URL**章节。
...
String endTime = String.valueOf( System.currentTimeMillis()/1000 + (60 * 60));//一个小时后
...
```
//2.生成SignKey Java示例
SecretKeySpec signingKey = new SecretKeySpec("自己的SECRETKEY".getBytes("utf-8"), "HmacSHA1");
Mac mac = Mac.getInstance("HmacSHA1");
mac.init(signingKey);
byte[] rawHmac = mac.doFinal(KeyTime.getBytes("utf-8"));
String SignKey = Hex.encodeHexString(rawHmac);
